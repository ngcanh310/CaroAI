...
public class State {
	...
	public void printState() {
		for (int i = 0; i < Value.SIZE; i++) {
			for (int j = 0; j < Value.SIZE; j++) {
				if (cell[i][j] == Value.AI_VALUE)
					System.out.print("- ");
				else if (cell[i][j] == Value.USER_VALUE)
					System.out.print("+ ");
				else
				...
				if (cell[x][y] == player) { // Nếu ô này đã được player chọn => kiểm tra
					for (int i = 0; i < 4; i++) { // kiểm tra 4 đường
						...
						for (int j = 1; j <= 4; j++) { // kiểm tra 4 ô tiếp theo
							...
							if (vtx < 0 || vty < 0 || vtx >= Value.SIZE || vty >= Value.SIZE)
								break;
							if (cell[vtx][vty] == player)
								count++;
							else
								break;
						}
						if (count == 5)
							return true; // Player thắng
					}
				}
			}
		}
	...
	public boolean isOver() {
		if (this.steps == Value.SIZE * Value.SIZE)
			return true;
		else
			return false;
	}
