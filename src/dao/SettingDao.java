...
public class SettingDao {
	public static Setting LoadSettingInfo() {
		...
		if (!file.exists()) {
			try {
				...
				return new Setting(Value.BACKGROUND_COLOR, Value.CELL_COLOR, Value.USER_TEXT_COLOR, Value.AI_TEXT_COLOR,
						Value.DEFAULT_MODE);
			}
		}
		try {
			...
			Color backgroundColor = new Color(Integer.parseInt(rem[0]), Integer.parseInt(rem[1]),
					Integer.parseInt(rem[2]));
	...
	public static void SaveSettingInfo(Setting setting) {
		...
		try {
			...
			// String color =
			// String.valueOf(rem.getRed())+","+String.valueOf(rem.getGreen())+","+String.valueOf(rem.getBlue());
			// writer.write(color + "\n");
			writer.close();
		} catch (Exception e1) {
