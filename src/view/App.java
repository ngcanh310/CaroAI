...
public class App extends JFrame implements MouseListener {
	 ...
	 */
	public void newGame() {
		// cài đặt màu nền, màu ô trong game, LoadSettingInfo đọc giữ liệu màu được lưu
		// trong setting.txt, setting.txt thay đổi mỗi khi thay màu bởi hàm
		// SaveSettingInfo trong SettingDao
		setting = SettingDao.LoadSettingInfo();
		caro = new CaroAI(setting.getMode());
		userClickedCell = null;
		aiClickedCell = null;
		// cài đặt hiển thị của introduction
	...
	public App() {
		...
		setSize(Value.SIZE * Value.CELL_WIDTH + 3 * Value.MARGIN + 280, Value.SIZE * Value.CELL_WIDTH + 50);// kích
																											// thước của
																											// mỗi phần
																											// thay đổi
																											// tự động
		...
		view.setBounds(Value.SIZE * Value.CELL_WIDTH + 2 * Value.MARGIN, Value.MARGIN, 274,
				Value.SIZE * Value.CELL_WIDTH);
		...
		btnNewGame.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				int result = JOptionPane.showConfirmDialog(null, "Bạn có muốn chơi mới?", "Xác nhận",
		...
		btnExitGame.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				int result = JOptionPane.showConfirmDialog(null, "Bạn có muốn đóng trò chơi?", "Xác nhận",
						JOptionPane.YES_NO_OPTION);
				if (result == JOptionPane.YES_OPTION)
		...
		rdbtnUserPlaysFirst.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				if (setting.getMode() == 1) {
					int result = JOptionPane.showConfirmDialog(null, "Xác nhận đổi chế độ chơi?", "Xác nhận",
							JOptionPane.YES_NO_OPTION);
					if (result == JOptionPane.YES_OPTION) {
